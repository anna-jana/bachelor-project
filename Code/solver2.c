...
struct GData compute_g_data(struct Data *data, double T) { // TODO
    struct GData ans;
    // compute SM g_*
    ans.g_s = T < T_min ?
    ...
    double d_T_nu_dT;
    if(T < 1e-2 * 1e6) {
        T_neutrino = pow(4.0 / 11, 1/3.) * T;
        d_T_nu_dT = pow(4.0 / 11, 1/3.);
    } else {
        double f = pow(8*(ans.g_s - 2) / (7.*6), 1./3);
        if(f < 1.0) {
            T_neutrino = f * T;
            d_T_nu_dT = 1 / 3.0 * pow(8 / 7. / 6. * ans.dg_s, 1. / 3 - 1) * T + f;
        } else {
            T_neutrino = T;
            d_T_nu_dT = 1.0;
        }
    ...
    double g_rho_neutrino_1 = 7 / 8. * 6 * pow(T_neutrino / T, 4);
    double g_s_neutrino_1 = 7 / 8. * 6 * pow(T_neutrino / T, 3);

    ans.g_s += data->parameter.Delta_N_eff * 7 / 8. * pow(T_neutrino / T, 3);
    ans.g_rho += data->parameter.Delta_N_eff * 7 / 8. * pow(T_neutrino / T, 4);
    ans.dg_s += data->parameter.Delta_N_eff * 7 / 8. * 3 * pow(T_neutrino / T, 3 - 1) * (d_T_nu_dT / T - T_neutrino / (T*T));
    ans.dg_rho += data->parameter.Delta_N_eff * 7 / 8. * 4 * pow(T_neutrino / T, 4 - 1) * (d_T_nu_dT / T - T_neutrino / (T*T));
    // dd?
...
/***************************** axion mass **********************/
double m_a(struct Data* data, double T) { // TODO:
    double m_a_0 = sqrt(pow(data->parameter.mu, 3) * data->parameter.micro_quark_mass) / data->f_a;
    if(T < data->parameter.mu) {
        // use T = 0 case
        return m_a_0;
    } else {
        return pow(data->parameter.zeta, data->parameter.N_f / 2) * pow(data->parameter.kappa, data->parameter.N_mu) * sqrt(data->parameter.c_N / (2 * data->parameter.eta)) *
            pow(data->parameter.m_psi / data->parameter.mu, data->parameter.N_f / 2) * pow(data->parameter.mu / (M_PI * T), data->parameter.eta) *
            (data->parameter.mu * data->parameter.mu / data->f_a) * pow(log(M_PI * T / data->parameter.mu), data->parameter.N_mu);
    }
}
...
int rhs(double _T, double y[], double dydT[], void* _params) {
    ...
    struct GData gdata = compute_g_data(data, T);

    // compute g quantaties

    double rho = M_PI*M_PI / 30.0 * gdata.g_rho * pow(T, 4);
    double H = sqrt(rho / (3 * M_pl * M_pl));

    double dtdT = - sqrt(8 * M_PI) * M_pl * sqrt(45 / (64 * pow(M_PI, 3))) *
        1 / (pow(T, 3) * gdata.g_s * sqrt(gdata.g_rho)) *
        (T * gdata.dg_rho + 4 * gdata.g_rho);
    double d2tdT2 = - sqrt(8 * M_PI) * M_pl * sqrt(45 / (64 * pow(M_PI, 3))) * (
            - (3 * T * T * gdata.g_s * sqrt(gdata.g_rho) + pow(T, 3) * gdata.dg_s * sqrt(gdata.g_rho) + pow(T, 3) * gdata.g_s * gdata.dg_rho / (2 * sqrt(gdata.g_rho)))
              / pow(pow(T, 3) * gdata.g_s * sqrt(gdata.g_rho), 2)
              * (T * gdata.dg_rho + 4 * gdata.g_rho)
            + (gdata.dg_rho + T * gdata.ddg_rho + 4 * gdata.dg_rho) /
              (pow(T, 3) * gdata.g_s * sqrt(gdata.g_rho))
    );
    dtdT *= temperature_unit;
    d2tdT2 *= temperature_unit * temperature_unit;
    double m = m_a(data, T / 4);
...
double solver(double Delta_N_eff, double mu, double micro_quark_mass, double zeta, double N_f, double N_mu, double kappa, double c_N, double eta, double m_psi,
        double T_osc, double theta_i, double f_a) {
    struct Data data;

    data.parameter.Delta_N_eff = Delta_N_eff;
    data.parameter.mu = mu;
    data.parameter.micro_quark_mass = micro_quark_mass;
    data.parameter.zeta = zeta;
    data.parameter.N_f = N_f;
    data.parameter.N_mu = N_mu;
    data.parameter.kappa = kappa;
    data.parameter.c_N = c_N;
    data.parameter.eta = eta;
    data.parameter.m_psi = m_psi;
    ...
    while(1) {
        ...
        for(int i = 0; i < N; i++) {
            ...
            double dtdT = - sqrt(8 * M_PI) * M_pl * sqrt(45 / (64 * pow(M_PI, 3))) *
                1 / (pow(T, 3) * gdata.g_s * sqrt(gdata.g_rho)) *
                (T * gdata.dg_rho + 4 * gdata.g_rho);
            n_over_s[i] = 45 / (2 * M_PI * M_PI) * data.f_a * data.f_a / (m * gdata.g_s * pow(T, 3)) *
        ...
        if(fabs(d_n_over_s_dT / n_over_s_avg) < eps && zero_crossings > num_crossings) {
            ...
            double rho_a_today = m_a(&data, T0 / 4) * n_a_today;
